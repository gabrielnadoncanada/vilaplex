...
>
    <div class="relative group">
        <div @class([
            ...
            'dark:bg-white/5 dark:border-gray-600',
        ])>
        ...
        <div @class([
            'absolute top-0 right-0 h-6 divide-gray-300 rounded-bl-lg rounded-tr-lg border-gray-300 border-b border-l overflow-hidden rtl:border-l-0 rtl:border-r rtl:right-auto rtl:left-0 rtl:rounded-bl-none rtl:rounded-br-lg rtl:rounded-tr-none rtl:rounded-tl-lg hidden opacity-0 group-hover:opacity-100 group-hover:flex transition ease-in-out duration-250',
            'dark:border-gray-600 dark:divide-gray-600',
        ])>

            <button
                x-init
                x-tooltip.raw.duration.0="{{__('filament.menu.items.edit')}}"
                type="button"
                wire:click="editItem('{{ $statePath }}')"
                class="p-1"
                title="{{__('filament.menu.items.edit')}}"
            >
                @svg('heroicon-o-pencil-square', 'w-3 h-3')
            </button>
            <button
                x-init
                x-tooltip.raw.duration.0="{{__('filament.menu.items.add-child')}}"
                type="button"
                wire:click="addChild('{{ $statePath }}')"
                class="p-1"
                title="{{__('filament.menu.items.add-child')}}"
            >
            ...
            <button
                x-init
                x-tooltip.raw.duration.0="{{__('filament.menu.items.remove')}}"
                type="button"
                wire:click="removeItem('{{ $statePath }}')"
                class="p-1"
                title="{{__('filament.menu.items.remove')}}"
            >
    ...
    <div x-show="open" x-collapse class="ml-6">
        ...
        >
            @foreach ($item['children'] as $uuid => $child)
                <x-filament.forms.components.nav-item :statePath="$statePath . '.children.' . $uuid" :item="$child" />
            @endforeach
        </div>
    </div>
</div>
